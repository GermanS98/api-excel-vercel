              ...
              datos.map((item: Bono, index) => ( // <-- 1. Se añade el 'index' aquí
                <tr 
                  ...
                  style={{ 
                    ...
                    backgroundColor: index % 2 !== 0 ? '#1036E20D' : 'transparent' 
                  }}
                  >
...
export default function TamarPage() {
   ...
   useEffect(() => {
    ...
    const maxDiasDelSegmento = useMemo(() => {
        if (bonosTamar.length === 0) return 1000;
        const maxDias = Math.max(...bonosTamar.map(b => b.dv));
    ...
    const datosParaGrafico = useMemo(() => {
      return bonosTamar.filter(b => b.dv >= rangoDias[0] && b.dv <= rangoDias[1]);
    }, [bonosTamar, rangoDias]);

    const datosParaTabla = useMemo(() => {
      return [...bonosTamar].sort((a, b) => new Date(a.vto).getTime() - new Date(b.vto).getTime());
    }, [bonosTamar]);
